...
function Feedback(props) {
  const [isHidden, setIsHiddedn] = useState(true);
  const [isVisible, setIsVisible] = useState(false);
  const [isFeedbackAvailable, setIsFeedbackAvailable] = useState(
    Number(localStorage.getItem('isFeedbackLeft'))
  );
  const [showThanks, setShowThanks] = useState(false);
  const [rate, setRate] = useState(50);
  const [comment, setComment] = useState('');
  const {
    actions: { onLeaveFeedback },
  } = props;
      ...
      {showThanks ? (
        ...
        <>
          <div className="Feedback__rate-container">
            ...
            <Button
              info
              label={constants.common.actions.send}
              onClick={onSendClick}
            />
          </div>
