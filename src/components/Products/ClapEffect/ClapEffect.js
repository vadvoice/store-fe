...
import './ClapEffect.scss';

let timer = null;
class ClapEffect extends React.Component {
   constructor(props) {
      super(props)
      const { product: { productVote }, user } = props;
      this.state = {
         count: 0,
         countTotal: productVote ? productVote.voteCount : 0,
         isClicked: productVote ? productVote.userIds.includes(user.userId) : false,
      }
   ...
   _handleClick() {
      ...
      timer = setTimeout(() => {
         const { product: { productVote } } = this.props;
         if (!productVote) {
            return;
         }
         this.props.actions.makeVote(productVote._id, {
            voteCount: this.state.countTotal
         })
      }, 3000)
   }
...
function getAppContent(count, countTotal, isClicked, handleClick) {
   return <div className="ClapEffect">
