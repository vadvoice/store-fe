...
function EditingСard(props) {
  const {
    product: { _id, imageUrl, title, description, amount, mark },
    actions: { onSubmit, onDelete, markProduct, onGalleryItemDelete },
    isEdit,
  } = props;
    ...
    return (
      <div>
        ...
        <div>
          ...
          {_id ? (
            <Button label={'delete'} onClick={() => onDelete(_id)} danger />
          ) : null}
          {markProduct ? (
            <Button
  ...
  const editGallery = (gallery = []) => {
    return (
      <div className="EditingСard__form__gallery">
        {gallery.map((galleryItem) => {
          return (
            <div
              key={galleryItem._id}
              className="EditingСard__form__gallery__item"
            >
              <i
                onClick={() => {
    ...
    return (
      <Form
        ...
        render={({ handleSubmit, values }) => (
          <form className="EditingСard__form" onSubmit={handleSubmit}>
            <FormField
              name={'title'}
              placeholder={'title'}
              label={'title'}
            ...
            <FormField
              ...
              validate={required}
            />
            <FormField name={'image'} label={'image'} component={ImageInput} />
            <FormField
              name={'gallery'}
              label={'gallery'}
              multiple
